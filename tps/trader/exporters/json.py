...
class JSONExporter(BaseExporter):
    ...
    def _do_export(self):
        def export_resources_to_path(prefix):
            for resource in self.revision.resource_set.all():
                self.extract_from_storage_to_path(
                    resource.file, os.path.join(prefix, resource.name)
                )
        ...
        self.write_to_file("statement.md", self.revision.statement_set.get().content)

        # Exporting problem files
        self.create_directory(self.OTHER_FILES_DIR_NAME)
        for file in self.revision.problem.files.all():
            self.extract_from_storage_to_path(
                file, os.path.join(self.OTHER_FILES_DIR_NAME, file.name)
            )
        ...
        for testcase in self.revision.testcase_set.all():
            if (
                not testcase.input_file_generated()
                or not testcase.output_file_generated()
            ):
                ignored_testcases.append(testcase)
                logger.warning(
                    "Testcase {} couldn't be generated. Skipping".format(testcase.name)
                )
                continue
            ...
            self.extract_from_storage_to_path(
                ...
                os.path.join(
                    self.TESTS_DIR_NAME,
                    "{testcase_name}.in".format(
                        testcase_name=generate_clean_name(testcase.name)
                    ),
                ),
            )
            self.extract_from_storage_to_path(
                ...
                json.dumps(
                    {
                        "score": subtask.score,
                        "testcases": [
                            generate_clean_name(t.name) for t in subtask.testcases.all()
                        ],
                    }
                ),
            )
        ...
        for solution in self.revision.solution_set.all():
            if solution.verdict:
                solution_dir = os.path.join(
                    self.SOLUTION_DIR_NAME, generate_clean_name(solution.verdict.name)
                )
            else:
                solution_dir = os.path.join(self.SOLUTION_DIR_NAME, "unknown_verdict")
            self.create_directory(solution_dir)
            self.extract_from_storage_to_path(
                solution.code, os.path.join(solution_dir, solution.name)
            )
        ...
        for resource in self.revision.checker_set.all():
            self.extract_from_storage_to_path(
                resource.file, os.path.join(self.CHECKER_DIR_NAME, resource.name)
            )
        ...
        if checker is not None:
            self.extract_from_storage_to_path(
                ...
                os.path.join(
                    ...
                    "checker{ext}".format(ext=os.path.splitext(checker.name)[1]),
                ),
            )
        ...
        for validator in self.revision.validator_set.all():
            ...
            for dir in dirs:
                ...
                self.extract_from_storage_to_path(
                    validator.file, os.path.join(full_dir, validator.name)
                )
        ...
        try:
            print(
                subprocess.check_output(
                    ["tps", "make-public"],
                    cwd=self.get_absolute_path("repo"),
