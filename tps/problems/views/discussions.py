...
class DiscussionsListView(ProblemObjectView):
    def get(self, request, problem_code, revision_slug):
        return render(
            request,
            "problems/discussions_list.html",
            context={"discussions": self.problem.discussions.all()},
        )
...
class DiscussionAddView(ProblemObjectView):
    ...
    def post(self, request, problem_code, revision_slug):
        form = DiscussionAddForm(
            request.POST, request.FILES, problem=self.problem, owner=request.user
        )
        if form.is_valid():
            obj = form.save()
        ...
        return render(request, "problems/add_discussion.html", context={"form": form})
...
class CommentListView(ProblemObjectView):
    ...
    def post(self, request, problem_code, revision_slug, discussion_id):
        discussion = get_object_or_404(
            Discussion, problem_id=self.problem.id, id=discussion_id
        )
        form = CommentAddForm(
            request.POST, request.FILES, owner=request.user, topic=discussion
        )
        if form.is_valid():
            obj = form.save()
            return HttpResponseRedirect(
        ...
        return render(
            request,
            "problems/discussion_comments.html",
            context={
