...
class ProblemObjectView(View):
    ...
    def as_view(cls, **initkwargs):
        ...
        def view(request, problem_code, revision_slug, *args, **kwargs):
            ...
            self.problem, self.branch, self.revision = extract_revision_data(
    ...
    def exception_occured(self, request, exception, *args, **kwargs):
        ...
        return render(
            request, "problems/exception.html", context={"exception": str(exception)}
        )
...
class RevisionObjectView(ProblemObjectView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if (
            request.method.lower() in self.http_method_names_requiring_edit_access
            and not self.edit_allowed()
        ):
    ...
    def delete(self, request, problem_code, revision_slug, *args, **kwargs):
        ...
        if not self.object_type:
            raise ImproperlyConfigured(
                "you must specify an object type for delete view"
            )
        if not self.redirect_to:
            raise ImproperlyConfigured(
                "you must specify a url for redirect to after delete"
            )
...
class ProblemObjectAddView(RevisionObjectView):
    ...
    def post(self, request, problem_code, revision_slug, *args, **kwargs):
        ...
        if form.is_valid():
            obj = form.save()
            return HttpResponseRedirect(
                self.get_success_url(request, problem_code, revision_slug, obj)
            )
        return self._show_form(request, form)

    def get(self, request, problem_code, revision_slug, *args, **kwargs):
        form = self.model_form(
            problem=self.problem, revision=self.revision, owner=request.user
        )
        return self._show_form(request, form)
...
class ProblemObjectEditView(RevisionObjectView):
    ...
    def post(self, request, problem_code, revision_slug, *args, **kwargs):
        ...
        if settings.DISABLE_BRANCHES:
            ...
            patch_number = 0
            for x in NewProblemBranch.objects.filter(name__startswith=patch_prefix):
                try:
            ...
            new_branch = self.problem.branches.create(
                name=new_branch_name, head=edited_on_commit_id
            )
                    ...
                    try:
                        ...
                        messages.error(
                            ...
                            _(
                                ...
                                "The merge should be done manually."
                            ).format(commit_id=str(new_commit)),
                        )
...
class ProblemObjectShowSourceView(RevisionObjectView):
    ...
    def post(self, request, problem_code, revision_slug, **kwargs):
        ...
        instance = get_git_object_or_404(
            self.model, pk=instance_pk, problem=self.revision
        )
    ...
    def get(self, request, problem_code, revision_slug, **kwargs):
        ...
        file_.open()
        code = file_.read()
        lang = getattr(instance, self.language_field_name) or detect_language(
            instance_pk
        )
        title = str(instance)
        return render(
...
class ProblemObjectDownloadView(RevisionObjectView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response["Content-Disposition"] = "attachment; filename=%s" % self.get_name(
            request, *args, **kwargs
        )
