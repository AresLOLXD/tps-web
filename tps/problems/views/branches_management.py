...
from problems.forms.version_control import (
    BranchCreationForm,
    ChooseBranchForm,
    CommitForm,
    MergeRequestAddForm,
...
__all__ = [
    ...
    "CreateMergeRequest",
    "MergeRequestList",
    "MergeRequestDiscussionView",
    "MergeRequestChangesView",
    "MergeRequestReopenView",
    "UnfollowMergeRequestView",
    "FollowMergeRequestView",
    "DeleteBranchView",
    "BranchesListView",
]
...
def branch_pull(request, source, destination):
    ...
    if pull_result:
        messages.success(
            request,
            _(
                "No conflicts occurred when pulling from {}. Committed automatically"
            ).format(source.name),
        )
        return True
    else:
        messages.error(
            request, _("Conflicts occurred when pulling from {}. ").format(source.name)
        )
        return False
...
def assert_no_open_merge_request(request, problem, revision_slug, source, destination):
    merge_requests = MergeRequest.objects.filter(
        source_branch=source, destination_branch=destination, status=MergeRequest.OPEN
    )
...
class CreateBranchView(ProblemObjectView):
    def get(self, request, *args, **kwargs):
        ...
        return render(request, "problems/create_branch.html", {"form": form})
...
def assert_not_changed_working_copy(func):
    def wrapper(self, request, *args, **kwargs):
        if self.branch.working_copy_has_changed():
            messages.error(request, _("Commit or discard your changes first."))
            return HttpResponseRedirect(
                reverse(
                    "problems:commit",
                    kwargs={
...
class PullBranchView(BranchControlView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            branch_pull(
                request,
                source=form.cleaned_data["source_branch"],
        ...
        return render(request, "problems/pull_branch.html", {"form": form})
...
class CommitWorkingCopy(BranchControlView):
    def post(self, request, problem_code, revision_slug):
        ...
        if (
            self.branch.working_copy_has_changed()
            and self.revision == self.branch.working_copy
        ):
            ...
            commit_form = commit_form_class(
                request.POST, instance=self.branch.working_copy
            )
            if commit_form.is_valid():
                ...
                if settings.DISABLE_BRANCHES:
                    if branch_pull(
                        request,
                        source=self.problem.get_master_branch(),
                        destination=self.branch,
                    ):
                ...
                if (
                    "create_merge_request" in request.POST
                    and not settings.DISABLE_BRANCHES
                ):
                    return HttpResponseRedirect(
                        reverse(
                            "problems:create_merge_request",
                            kwargs={
                                "problem_code": self.problem.code,
                                "revision_slug": self.branch.get_slug(),
                            },
                ...
                return render(
                    request,
                    "problems/confirm_commit.html",
                    context={"changes": changes, "commit_form": commit_form},
                )
    ...
    def get(self, request, problem_code, revision_slug):
        ...
        commit_form = commit_form_class(
            instance=self.branch.working_copy,
            initial={"commit_message": initial_commit_message},
        )
...
class ConflictsListView(BranchControlView):
    def get(self, request, problem_code, revision_slug):
        ...
        return render(
            request,
            "problems/conflicts.html",
            context={"conflicts": self.revision.merge_result.conflicts.all()},
        )
...
class ResolveConflictView(BranchControlView):
    ...
    def get(self, request, problem_code, revision_slug, conflict_id):
        ...
        return render(
            request,
            "problems/show_conflict_diff.html",
            context={
                "conflict": conflict,
...
class CreateMergeRequest(BranchControlView):
    def get(self, request, *args, **kwargs):
        ...
        if result:
            return result
        differences = get_revision_difference(base=master.head, new=self.revision)
        master_merge_base = self.revision.find_merge_base(master.head)
        default_description = "\n".join(
            [
                "* " + revision.commit_message
                for revision in self.revision.path_to_parent(master_merge_base)
            ]
        )
        ...
        return render(
            request,
            "problems/create_merge_request.html",
            context={"differences": differences, "form": form},
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            differences = get_revision_difference(
                base=self.problem.get_master_branch().head, new=self.revision
            )
            return render(
                request,
                "problems/create_merge_request.html",
...
class MergeRequestList(ProblemObjectView):
    def get(self, request, *args, **kwargs):
        ...
        merge_requests = MergeRequest.objects.filter(
            source_branch__problem=self.problem
        )
        if not show_closed:
            merge_requests = merge_requests.filter(status=MergeRequest.OPEN)
        return render(
            request,
            "problems/merge_requests_list.html",
            context={"show_closed": show_closed, "merge_requests": merge_requests},
        )
...
class MergeRequestDiscussionView(ProblemObjectView):
    def get(self, request, problem_code, revision_slug, merge_request_id):
        ...
        form = CommentAddForm(owner=self.request.user, topic=merge_request)
        return render(
            request,
            "problems/merge_request_discussion.html",
            context={
    ...
    def post(self, request, problem_code, revision_slug, merge_request_id):
        ...
        if merge_request.status != MergeRequest.OPEN:
            messages.error(
                request,
                _("This merge request has already been either closed or merged"),
            )
            ...
            form = CommentAddForm(
                request.POST, owner=self.request.user, topic=merge_request
            )
            if form.is_valid():
                form.save()
                return HttpResponseRedirect(
...
class MergeRequestChangesView(ProblemObjectView):
    def get(self, request, problem_code, revision_slug, merge_request_id):
        ...
        return render(
            request,
            "problems/merge_request_changes.html",
            context={
                "merge_request": merge_request,
                "differences": differences,
            },
...
class DeleteBranchView(BranchControlView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        diverged_from_master = (
            self.revision.find_merge_base(self.problem.get_master_branch().head)
            != self.revision
        )
        return render(
            request,
            "problems/confirm_delete_branch.html",
            context={"not_merged_changes": diverged_from_master},
        )
